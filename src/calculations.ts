...
export function calculate(pkg: Package): CalculationResult {
  ...
  // Kørselsfradrag is a ligningsmæssigt fradrag — it only reduces kommuneskat, not bundskat
  const kørselsfradragSaving = pkg.yearlyKørselsfradrag * KOMMUNESKAT_RATE;
  const annualTakeHome = Math.max(taxableAnnual - taxBreakdown.total + kørselsfradragSaving + postTaxAdjustmentAnnual - annualCommuteCost, 0);
